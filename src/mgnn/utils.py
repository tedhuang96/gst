...
def load_dataset(args, pkg_path, subfolder='train', num_workers=4, shuffle=None):
    result_filename = args.dataset+'_dset_'+subfolder+'_trajectories.pt'
    dataset_folderpath = join(pkg_path, 'datasets', args.dataset)
...
def args2writername(args):
    writername = str(args.num_epochs)+'-'+str(args.spatial)\
...
def arg_parse():
    ...
    parser.add_argument('--temporal', default='lstm',
                        help='temporal encoding methods: lstm, temporal_convolution_net.')                    
    ...
    parser.add_argument('--temporal_num_layers', type=int, default=6)
