...
class EdgeSelector(nn.Module):
    ...
    def forward(self, x, A, attn_mask, tau=1., hard=False, device='cuda:0'):
        ...
        x = self.norm_node(x)
        x = x * attn_mask_ped
        x_ghost = torch.zeros(bsz, 1, d_model).to(device)
        x_neighbor = torch.cat((x, x_ghost), dim=1)
        x_neighbor = torch.ones(bsz,nnode+1,nnode,d_model).to(device)*x_neighbor.view(bsz,nnode+1,1,d_model)
        x_target = torch.ones(bsz,nnode+1,nnode,d_model).to(device)*x.view(bsz,1,nnode,d_model)
        x_neighbor_target = torch.cat((x_neighbor, x_target), dim=-1)
        A = self.norm_edge(A)
        A = A * attn_mask.permute(0,2,1).unsqueeze(-1)
        A_ghost = torch.zeros(bsz, 1, nnode, 2*d_model).to(device)
        A = torch.cat((A, A_ghost), dim=1)
        A = torch.cat((x_neighbor_target, A), dim=-1)

        attn_mask_ghost = torch.ones(bsz, nnode, 1).to(device)
        attn_mask = torch.cat((attn_mask, attn_mask_ghost), dim=2)
        attn_mask_neighbors = attn_mask.view(bsz, nnode, nnode+1, 1) * attn_mask.view(bsz, nnode, 1, nnode+1)
        attn_mask_neighbors = attn_mask_neighbors.view(bsz*nnode, nnode+1, nnode+1)
        ...
        A_perm = A_perm.reshape(bsz*nnode, nnode+1, 4*d_model)
        A_perm = A_perm.permute(1,0,2)
        
        _, _, A2 = self.self_attn(A_perm, A_perm, A_perm, attn_mask=attn_mask_neighbors) 
        A2 = A2.reshape(bsz, nnode, self.nhead, nnode+1, self.head_dim)
