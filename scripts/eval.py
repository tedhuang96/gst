...
from src.mgnn.utils import arg_parse, load_dataset, average_offset_error, final_offset_error, \
    negative_log_likelihood, args2writername
...
def main(args):
    ...
    loader_test = load_dataset(args, pathhack.pkg_path, subfolder='test')
    ...
    test_aoe, test_foe, test_aoe_std, test_foe_std = inference(loader_test, model, args, mode='test', tau=0.03, device=device)
    print('dataset: {0} | test aoe: {1:.4f} | test aoe std: {2:.4f} | test foe: {3:.4f} | test foe std: {4:.4f}'\
                        .format(args.dataset, test_aoe, test_aoe_std, test_foe, test_foe_std))
...
def inference(loader, model, args, mode='val', tau=1., device='cuda:0'):
    with torch.no_grad():
        ...
        aoe_sum_epoch, aoe_std_epoch, foe_sum_epoch, foe_std_epoch = [], [], [], []
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if mode == 'val':
                ...
                for _ in range(20):
                    ...
                    if args.deterministic:
                        ...
                        loss_tmp = negative_log_likelihood(gaussian_params_pred, v_pred_gt, loss_mask=loss_mask_per_pedestrian)
            ...
            if mode == 'val':
                ...
                loss_epoch.append(loss.detach().to('cpu').item())
                aoe_sum_epoch.append(aoe_sum.item())
                foe_sum_epoch.append(foe_sum.item())
        ...
        if mode == 'val':
            ...
            aoe = sum(aoe_sum_epoch)/loss_mask_epoch.sum()
            foe = sum(foe_sum_epoch)/loss_mask_epoch.sum()
            aoe_std = sum(aoe_std_epoch)/loss_mask_epoch.sum()
            foe_std = sum(foe_std_epoch)/loss_mask_epoch.sum()
            return aoe, foe, aoe_std, foe_std
        else:
