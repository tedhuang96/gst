...
only_observe_full_period=--only_observe_full_period
...
for random_seed in 1000; do
    for dataset in eth hotel univ zara1 zara2; do
        python -u scripts/train.py --spatial $spatial --temporal $temporal --lr $lr\
            ...
            | tee -a logs/train_full_connection_fully_observed.txt
    done
done